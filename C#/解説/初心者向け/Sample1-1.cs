...
namespace CSharpSample1_1
{
    ...
    public partial class Sample1_1
    {
        ...
        /// は全部C++で言うところの参照渡しになるのだが、実際は値渡しと呼ばれている。
        /// (参照渡しじゃなきゃ、超パフォーマンスに問題あるのでそんなことをするわけがない)
        /// ※なぜ値渡しと呼ぶべきかを長々と記載して力説するサイトがあるが、
        ///   そもそもグローバル変数とかと同じで、その言語における固有の呼び方なんてどうでもいいです。
        ///   コンピュータの一般的な仕組み、用語に照らし合わせて考えるべきです。(最近は少数派なのかも)
        /// 
        /// で、参照型を引数に渡す場合、引数の値を変更しても原則としては呼び出し元のインスタンスに影響を与えません。
        /// ※さすがにそんな無茶なことは出来ないように規制がかかっている
        /// 
        /// もし、その引数自体を書き換えたかったらrefを付ければいい。呼び出し元もref付けなきゃいけないけどね。
        /// といってもこのrefって有効に使う局面があんま思いつかない。
        /// ※ 多分構造体の場合に威力を発揮するじゃないかな？
        /// また、値を変更するだけでなくnewにより新たなインスタンスを与えることができるので注意。
        /// 
        /// また、C++をやっていた人間だと誤解しがち(ネットにもそう考えてしまうような記事がある)なことがある。
        /// それは "ref(or この後のout)つけなければconst &と等価で、引数の内容変更無理なんでしょ？"ということ。
        /// 実は、C#の場合 refやoutが付いていなくても、参照型に限っては、以下の方法により呼び出し元のインスタンスに影響を与えることができます。
        /// 1. プロパティへの代入
        /// 2. publicのメンバ変数への代入
        ...
        /// その方はどうしても関数の中で変更したかったのでpublicにしたわけですが、そういう人がいるのは事実です。
        /// 言語的に可能なんですから、やる人間が居ても私は不思議に思いません。
        /// 
        /// Javaも似たようなことができるはずなので、なぜそれを禁止する構文を導入しないのか不思議です。
        /// ※7.2で入ったinパラメータは若干意味合いが違いそう
        /// 
        /// そういうのは普通の人はやらないし、気を付ければ対処出来るという考えの気はしますけど、
        /// それだったらC言語のポインタやC++の多重継承だって、同じですよね？
        /// その割には、そういうのは邪悪だし認めないって人が多すぎる気がする。
        /// </summary>
