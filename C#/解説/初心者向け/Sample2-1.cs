...
namespace CSharpSample2_1
{
    ...
    public class Hoge4 : Hoge3
    {
        ...
        /// overrideならインスタンスに応じて該当するメソッドを呼び出すが、newだと作成したインスタンスに応じて呼ばれるのが決定する
        /// 親クラスのインスタンスなら親クラスだし、派生クラスのインスタンスなら派生クラスのメソッド。
        /// 親クラスのメソッドにいれて回すポリモーフィズム目的なことをすると、派生クラスのメソッドは呼ばれない。
    ...
    /// プロパティや実装したメソッドは持てないが、プロパティは持てる(でも用意するのは派生クラスだけどｗ)
    /// でもなぁ、インターフェースだとprivate setとかに出来ない(privateで怒られる)から、使い道は・・・
    /// アクセシビリティはすべてpublic強制
    /// </summary>
    interface ISample
    {
    ...
    public class MulCalc : ISample
    {
        ...
        /// でも実装が見つからないとちゃんと怒られるから実装漏れかどうかはすぐわかる
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
