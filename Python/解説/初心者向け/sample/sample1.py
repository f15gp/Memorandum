...
def calculate_numeric():
    ...
    z = x - y       # 引き算
    z = x * y       # 掛け算
    z = x / y       # 割り算(ただし、結果は小数となる。Cとかみたいに切り捨て割り算で整数になるのとは違う)
                    # 実はpython2では他の言語と同じで切り捨ての動作だった
    z = x % y       # xをyで割った余り
    z = x ** y      # xのy剰
...
def compare():
    ...
    x if a else y  ってな書式。ま、あんま使わないけど
    """
    if "a" in "abc":    # inは右側に含まれていれば真
        print("in")
    if "d" not in "abc":# not inは右側に含まれてなければ真
        print("not in")
    if "abc" == "abc":
        print("文字==でOK")
    if 10 is 10:
        print("isは==と同じ。何が違うかな？")
    if 10 is not 11:
        print("is notは!=と同じ。何が違うかな？")
...
def like_array():
    ...
    for elem in a:      # 一個ずつ取り出す時はforで
        print(elem)
    # 文字列で説明した用法で取り出すことも可能(説明してなかったけど、あれはリストに入れてるので)
