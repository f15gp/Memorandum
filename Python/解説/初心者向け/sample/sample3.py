...
def around_function():
    ...
    # でも、Python3じゃnext()じゃ動かなくて怒られますんで、ご注意を。
    ...
    def decolater(func):                # デコレーター関数。funcはデコレートされる関数。
        def wrapper(*args, **kwargs):   # 実際のデコレート処理を書く関数
            print("--start--")
            func(*args, **kwargs)       # デコレートされる関数はここで実行。つまり前後にprintを加えるデコレーション。
            print("--end--")

        return wrapper                  # お約束

    @decolater  # decolater()って関数で次の関数をデコレートするよって意味(@decoならdeco()って関数でデコレートってこと)。
    def test():
        print("Hello")
...
def list_etc():
    ...
    # どうしても使う必要がある人は自分で調べてｗ
    # ・・・でも、内包記法あってのPython位の勢いで書いてる人が居るので、読めないといけないんだろうなぁ。
