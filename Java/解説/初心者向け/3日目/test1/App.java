...
public final class App {
     ...
     * また、1ファイルに"必ず"publicのクラスが1つなければいけないというわけではなく、未指定だけのクラスしかなくても問題ないしコンパイルも出来ます。
     * 逆にpublicの場合は必ずファイル名と同じクラス名でなければいけないので、publicのクラスは1ファイルに1つしか用意できません。
     ...
     * もう一度書きますが、クラスを原則publicにするということは、全てのクラス群(class、interface、enum etc)をパッケージの外に"公開"することを意味します。
     * これって広義のカプセル化に違反してると思うし、パッケージ外で使われるとパッケージの中の人たちの都合で改造することが出来なくなると思うのですが？
     * ※ 特にclassにfinalをつけてないとか、メソッドがデフォルトでオーバーライド可能(これはちょっと後のメソッドのところで書いてます)なJavaだと怖すぎるんですが。
     * C言語使っての開発で.libや.dllを作成する時、全てのdefine etcを公開用のヘッダに用意するとかありえないと思うんですが、
     * JavaだとなんでこれがOKなんですかね？不思議です。
     * ※ public付けるわけねぇだろ！無指定のクラスを1つずつ用意するんだYO。だったら、すみませーん。
     *
     * ただ、入門書でmainを用意するために必ずclassにpublicをつけたものを延々と使いまわされてしまうせいで、
     * きちんと理解しないまま、classにはpublicをつけるんだ。と誤解した例は結構あると思ってます。
     ...
     *    つまり、規格を決定する方たちもいきなりclassやstaticの説明してないのにmainのために必要というのは、初学者に教える上で問題だと考えていたってことですね。
     *    安心しました。
     *
     ...
     * ※ ネット上での議論とかの話です。何度も言ってますが、筆者はJavaの開発に参加したことないので、リアルの方々はそんなこと全く思ってないかもしれません。
     *    ただねぇ、今の方々(若い人だけではなく)はネットで調べて、ネットの言説を信じ込んでる傾向ありますからねぇ・・・
     * 多分今までの考え方を変えたくないって理由でしょう(もしかしたら、いわゆる宗教論争なのかもしれない)が、そんなもの害悪にしかなりません。
    ...
    public static void main(String[] args) {
        ...
        * synchronized: 同時に複数のスレッドが呼び出せないようにします。マルチスレッドで重要。あとで ry)
