...
public final class App {
     ...
     * Deprecatedのクラスを使用した場合、コンパイルしたときに警告が出ます。
     * ただし、Java標準のアノテーションとフレームワークなどが提供するアノテーションがありますが、筆者が書くのはJava標準のです。
     *
     * (アクセス修飾子)
     ...
     * 1ファイルに必ずpublicのクラスが1つなければいけないわけではなく、未指定だけのクラスしかなくてもコンパイル出来ます。
     * 逆にpublicの場合は必ずファイル名と同じクラス名でなければいけないので1ファイルに1つしか用意できません。
     * 1つより多く用意するとコンパイルエラーになります。
     * ちなみにpublicのクラスが1つだけなのは、確かJVMがロードを簡単にするためとかいう手抜き？理由だった記憶があります。
     * 決して、なにか明確な利点があるから採用とかいう理由では、ないです。
     *
     * 個人的見解ですが、よく聞く"1ファイル1クラス"というのは、publicのクラスはファイルに1つしか作成できないというのを誤解したのが始まり
     * だと思ってます。"1ファイル1クラスがわかりやすい"という方もいますが、実用的な開発してたら、そうならないんじゃないかと思ってます。
     * なぜかというと、このpublicの制約ってのはclass、interface、enum、record全てに適用されるわけですが、
     * 例えばわざわざenumをpublicにして一つだけ定義するファイルを全て用意してると思えません(え、まさかJavaの人たちって用意するの？)。
     * まぁ用意するんだとしたらファイル名の管理とかどうしてるんでしょうか？私の知ってる大規模開発とかだと、ファイルを新規に作るのって申請とか
     * 必要な現場が結構あるはずですがねぇ。
     * 
     * Javaには内部クラスや匿名クラスがあり、特に匿名クラスはラムダに発展するため、実質的にこれらも全てクラスだと私は思います。
     * 皆さんラムダを使うのどうするんでしょうかね？1ファイル1クラスなんでしょ？まさか、グローバル変数と同じで"class"がついてないのでクラス
     * ではない。とかいう迷言ｗがあるんでしょうか。
     * ちなみにC++やC#にはこの制約がありません。
     * でも、というか筆者がJavaを嫌う理由の1つでもあるのですが、Javaで育った？方は、このJavaの文法による制約・制限や
     * コーディングスタイルetcを合理的な理由なしに、他言語に適用しようとする人が多いと感じます。
     * 多分勉強したくないとか今までの考え方を変えたくないって理由でしょうが、そんなもの害悪にしかなりません。
     ...
     *
     * 今は、あまりpublicをつけたクラスはいらないんじゃないかと思ってます。
     * パッケージの外に公開するのはクラスでなく後で説明するinterfaceを基本にすべきじゃないでしょうか。
     * まぁ、そんなこと守ってる人は少なそうですが。
     * ただpublicでfinalもついてないクラスをポコポコ作成するのはやめてください。
     * 大規模開発なんかではpublicになってたらJavaの場合、勝手に使っていい＆勝手に継承していいいと受け取る人が多いでしょう。
     * 大規模な開発では、その辺をきちんと制御できない場合がほとんどです。
     * ※ 冗談ぬきですごい開発の仕方/解釈をする人がいらっしゃるので。
     *
     * (非アクセス修飾子)
     ...
     * 
     * また、現代というかstrictfpはとうの昔に意味を失っています。
     * 元々strictfpは、x86系かつSSE2というCPUの拡張命令がない時にIEEE754に厳格に従って演算を行うかどうかでしたが、
     * 今の時代にSSE2を実装していないCPUなどまずお目にかかれないでしょう。
     * まぁJava 1.0の頃は小数の計算がIEEE754でなく独自の計算で行っていたCPUが現役でしたので、そういったCPUではJavaが実行
     * 出来ないという笑い話にならない世界があったのも事実ですが。
     *
     * (extends)
     ...
     * 単独で使用するようなものではありません。
     * ※ 単独で使って、それこそグローバル変数になってる場合が多い、シングルトンというパターンがありますが、
     *   あれも正しく使われること少ないんですよね。そもそも、1つだけしか存在しないインスタンスってそう多くないですし。
     *
     * (implements)
    ...
    public static void main(String[] args) {
        ...
        * まぁ使う場合は既に吟味されてstaticになっていると思うので、気にしなくていいと思います。
        ...
        *
        * Javaは時代背景もあるけど、学者が定義したのか全メソッドがデフォルトでオーバーライド可能です。
        ...
        * 今はinterfaceの実装を中心とすると思うので、コーディング規則にない限りクラスをfinalにしてメソッドにはfinalをつけない
        * が楽じゃないかなと。
        * ただ、クラスにfinalをつけない場合は、メソッドごとにfinalをつけるか吟味すべきです。
    ...
    static void first() {
        var subA = new SubA();
        System.out.println(subA.Double(2));
    }
