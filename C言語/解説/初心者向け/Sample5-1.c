...
// qsort用 糞マジメに書く
// あ、そうそうqsortで渡す関数の引数はvoid*なのだけど、このvoid*を分かってなくて
// void*引数の関数を作って、関数内部で引数使う時にキャストして使う書き方をする人がいます。
// でも、void*はどんなポインタ型でも入れることが出来る(ただし、関数型のポインタは除く)ものなので、
// 普通に型に合ったのを定義してやれば↓のように不要なキャストなしで使えますぜ
...
// 構造体の中の文字列を辞書式順に並び替え
// こういうライブラリを用意するとqsortがvoid*であっても、関数の型チェックを受けることが出来るのでいいと思うんだけどなぁ
// ※このソースでは関数側から直接qsortを呼び出すので恩恵も糞もありゃしないが
