...
/************************** 変数 ***************************/
// 今だとビット幅が指定できる？(語弊あり)型が導入されているので、そちらを使うのが好みです。
// しかし、業務だとまず無理だと思います。なので普通のをグローバル変数の所で紹介しときます。
// あと、後で書くtypedefってものでWORD、BYTEなどの型が用意されていることが良くあります。
...
// ↓これがグローバル変数の一覧表となる。守られることは少ないが、グローバル変数は一つのファイルにまとめるべきさ
