...
void Sample41Func3(void)
{
	...
	// ポインタ渡しは2つ用途があります。
	// 一つは戻り値が一つしかない関係上、引数に渡す変数を変更してもらう場合です。↑のサンプルは引数1つですが、
	...
	// よってC#とかだとrefやoutが付いてなくてもプロパティとかに限っては、値の変更が出来てしまいます。
	// ・・・なんでC/C++では出来ないことを後継言語が出来るようにするかなぁ・・・
	//      どう考えても、呼び出した関数内でプロパティが変更できることに利点があると思えないのだが
...
void Sample41Func4(void)
{
	...
	// ※あと一つ、組み込みとかで良く使わている巨大な構造体だとスタックオーバーフローすることがあるので、
	...
	// 大昔はvoid*がなかったのでchar*を返してました。そのせいで
