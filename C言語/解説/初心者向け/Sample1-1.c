...
*				-# 新規作成
*			- 2020-09-03 Bunya
*				-# _countofの戻り値がunsigned __int64だったのでprintfの書式を変更。
*			- 2021-04-13 Bunya
*				-# ミスや誤解を招く内容を訂正。
*
**********************************************************************
...
#include "Sample1-1.h" // <-これをincludeしなくてもこのファイルのコンパイルは通ります。が、引数と戻り値がチェックされないのでダメです

/**
* @brief		引数のチェックをしないのサンプル。が、引数を渡されても引数名がないのでどうせ利用はできない
* @param[in]	なし
* @param[out]	なし
* @return		なし
* @detail       briefに全部書いてる。
...
void Sample11Func1()
{
	...
	// ちなみに、変数ですがローカル変数(関数の中とかに定義される変数)は初期値を与えない場合、不定値となります。
...
int Sample11Func2(void)
{
	...
	// が、少なくともC++でautoとかいう型推論を標準型に使う場合は非常に重要になります。
...
int Sample11Func3(int a)
{
	...
	// そうすると(~u)は0xffffffa5になってるので>>4した後にunsigned charに丸め込むと0xfaとなり、恐らく期待される0x0aとは異なるって話。
...
{
	static int a = 0;	// 関数を抜けても最後の演算値が保持される
	a++;
}
...
// このサンプルでは、main.cに公開するヘッダにプロトタイプを書いているのがアレレですが、普通は公開しないヘッダかこのファイルの先頭などに
...
void Sample11Func6(void)
{
	...
	// VSはVLAをサポートしてません。
	// VLAがサポートされない場合、配列のサイズは定数式で割り当てる必要がありますが、C言語ではconst変数は定数ではないのでダメです。
	// なので、恐らく仕事のC言語で見かけるのは、どうかなぁと書いた関数引数のポインタ書き換え阻止のみでしょう。
...
void Sample11Func7(void)
{
	...
	// 文字列含め、出来る限りこの形を推奨します。(ただし固定長配列をどうしても必要とする場合はそちらを優先する)
	int d[] = { 1, 2, 3, 4, 5 };
