...
extern const char c_loop_max;		// char型。C言語から伝統的に文字型とされていましたが、
...
extern const int i_loop_max;		// int型。CPUが最も扱いやすいサイズになる(規格上これだけ以上は表現できないとはなっているけど)
									// 16bit, 32bit, 64bit CPUでサイズが変わってしまうことに注意。
extern const long l_loop_max;		// long型。intより大きな数字を扱えれることになっているが現状intと違いがないはず
...
extern const char* str_loop_max;	// 文字列定数
...
// ※本当はC++なら文字列はstd::string、unsigned charはバイナリのバイトデータとして扱って、
//   charを文字に使うとかのはやめていただきたいのであるが。。。
//
