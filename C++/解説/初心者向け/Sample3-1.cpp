...
#pragma region array_vector_list
	...
	void func1()
	{
		...
		// あ、vectorとかは初期は少しだけしか領域を確保してないので、沢山追加をすると倍々ゲームに近いノリで動的確保を行います。
		// んで、C++03からvectorの領域は連続してることを保証しなければならなくった(つまり、リスト的な構造で領域確保はアウト)ので、
		// メモリの再配置、コピーなんかが必要になってきます。
		// 最初からこの程度は追加を行うってのが分かっているならreserveとかコンストラクタで指示してください。
		// それもせずにC#と比べて速度がとか書いてるサイトもあったりしてバカじゃないの？とか思ったり。
		// VMは最初からかなりの領域をがめてるんだから当たり前じゃんとかね。だから最初に起動するのに時間かかるわけで。。。
		...
		// 当然これらはスコープを外れる時に内部で確保した領域は自動的に解放される
	}
...
#pragma region map_unordered_map
	/// <summary>
	/// mapに独自オブジェクトを入れるならソートするために比較演算子が必要になる。
...
#if 1
		// #ifが0で↓の関数が死んじゃうとmapへのinsertでエラーが起きます。
		// なぜかというとmapのinsertを行う際に、<で比較する処理を見つけれなくなるからです。
		// mapに独自オブジェクトを突っ込みたかったら、operatorとかの知識が必須です。
	...
	/// <summary>
	/// map, map_unordered_mapのサン ry)
	/// </summary>
	void func3()
	{
		std::map<int, std::string> mapper{		// キーをint型、値をstringとした木構造。C#のDictionaryと同じ
			{3, "hoge"},
			{4, "hogehoge"},
			{6, "hopper"},
		};
		// えーと、mapは大抵pairと一緒に用いられて、かつキーをstringにして、valueを関数ポインタ(or std::function)にすると面白いことが出来る。
		//  ※以前送信したけど、多分誰も見てないだろうなっと
		// とりあえずここではそんなことせずにinsertを行う
		mapper.insert(std::make_pair<int, std::string>(1, "mapper"));

		// 一つ注意
		// mapは必ずデータ挿入のたびにソートするので、ソートが必要なかったらunordered_mapを使うこと
		...
		std::cout << "3rd:" << unordered.at("3rd") << std::endl;	// まぁfind使わなくてもキーを指定したらvalue取れるんだけどねｗ
	}
...
#pragma region unique_ptr
	...
	void func4(int placed_size)
	{
		...
		auto p = protected_overflow.get();		// get()するとポインタが取得できるんで。だってunique_ptrはrange-based-forできないんだもん。
		...
		// が逆に自動解放全般に言えるが、例えばforの中の処理で毎回宣言するように書いちゃいけません
...
#pragma region function
	...
	{
		// ま、まずはラムダについて述べてみようか。
		auto func_lamda = [](auto x){ return x + 2; };	// まぁ、最小のラムダ式じゃありませんがね
		...
		// 配列とかlistとか出来る。独自クラスも(実装すれば)使える。一応実装したものはクラスの所で説明予定
		...
		// 関数ファンクタがあったのは、昔だとそれが一番最適化がかかったからです。それと一部関数ポインタを指定できない、困ったチャンがいたから。
		// でも、関数ファンクタの問題は、1クラスに一つしか定義できないこと。だから、ファンクタの為だけに、適当な名前を考えるのが大変だし、プロジェクト規則
...
#pragma region thread_async
	...
	/// <summary>
	/// 単に0-99までの乱数を1000000回発生させてみるだけの処理
	/// </summary>
	void thread_func()
	{
