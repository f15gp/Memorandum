...
#pragma region 実際の使い方
	...
	void func1()
	{
		class_0 a;			// class_0のオブジェクトを用意しました
		a.a = 2;			// public変数なら直接代入したりできる
		a.func1(4);			// public関数はオブジェクト経由で呼び出せる
		...
		// スコープ抜けたのでここでunique_ptrは自動解放されてる
...
// また、C++のtupleは現在.NetのTuple(C# 7.0とは違うやつ)と同じで結構使いにくいです。
// なので共通データ型としての構造体は↓に述べる制約とは無関係です。
// むしろ共通データ型なので引数とかにそのまんまで適用するべきです。よって、以下はオブジェクトとして扱うならなのでお間違えの無いよう
...
struct hoge_data_only
{
	...
	// ま、あってもいいけど
	// そもそもゲッターとかセッターとかって、検索でのヒット率向上(でも、なんでもかんでも用意したら多分ヒット率低下する)とか、
	// C++みたいにstructやクラス外の変数、関数を用意できないって事情から始まったんじゃない？って気がする
	// 用意するならそうだな、排他が必要なデータを管理するなら用意するかなぁ。それならそれでmutex_set～なんて名前にするかも
...
// 例えば、ファイルを扱うとか単独のオブジェクトとして扱うもの
// オブジェクト一つ一つが個々のオブジェクトに写像するようなもの
...
// 基本的な状態遷移を受け持つオブジェクトがあるけど、実際の状態遷移、外部へのアクセスなどは個々のオブジェクトで少しずつ違うという場合に
