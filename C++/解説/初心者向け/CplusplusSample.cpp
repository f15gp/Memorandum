...
//         ただし、世の中の多くではスネーク記法ではないと思われる。
// 注意: C++03は原則考えない。C++11/14での書き方で記載予定。
...
//      #pragma region～endregion(ソースコードの任意箇所での折り畳み/展開が可能)使っても良かったのですが、まぁ控えてます。
...
//      ※ていうか今のC++は既にC言語の上位互換じゃないので、better Cとして使うこと自体に無理があるけどね
...
//
// 他にも理由はあるので、C++での業務では十分お気を付けを
//
// ちなみに、まず見かけませんが、本来ヘッダファイルは対象のファイルが読み込むだけのプライベートなヘッダと
...
//
// それと、C++の標準ヘッダは.hが付きません。#include <iostream>みたいな感じ(古い教科書だと.hついてたりするけど)
...
// ただ、C++17ではこの内容に変更が入ったという情報もありますが、C++17規格を読んでないのでわかりませんｗ
...
int main(int argc, char* argv[])
{
	...
	int ih = 5;	// 組み込み型なのであれだが、実際的にはコピーコンストラクタを呼び出すのと同義
