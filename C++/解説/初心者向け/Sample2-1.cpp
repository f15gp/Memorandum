...
#pragma region 構造体
	...
	void func2()
	{
		...
		// はっきり言います。やめろ、バカ野郎です。
		// POD以外(ただしPODという概念はC++20により非推奨の概念となります)の構造体に適用すると厄介なバグを引き起こすことがあります。
		// そしてクリア対象のサイズを間違えると一発で領域破壊ができます。
		// 構造体の全メンバを初期化しているなら↓で十分です。
		c = Struct();
		...
		// ※ C言語でも全メンバを{}内に指定するかC99以降の指示付き初期化を使えば可能。でもC99って・・・
...
#pragma region if_繰り返し
	...
	void func3()
	{
		...
		int array[elements];
#endif
		// ただ、配列は後から述べるstd::arrayが良いかもしれない。なぜならこいつは要素数を覚えてくれて、範囲外アクセスも見れるから。
		// また、配列は、C++03だとアルゴリズムというのを適用しにくいというのはあったが、C++11以降では配列への適用も簡単。
		// ※(arrayが入る前のvectorというものの話)[]に比べて、.at()が範囲外アクセスを検出して例外をだすものになるのだけど、
		//   ひと昔前は例外を出したり検査するので少し遅いということで皆[]ばかりを使ってました。(at()の存在そのものを知らないなど思いたくない)
		...
		// ただし、std::arrayは必ず要素数の指定が必要になるのがネック。
		//
		...
		// std::for_eachは指示した始点から最後までなめていってくれます。
...
#pragma region 例外
	...
	void func7()
	{
		...
		// だから、メモリがない状況でもなんとか動くようにって言われると、gotoで飛ばしてfree(or delete)しまくって、必要なことしたら再起動。
		// これの方がよっぽどマシってな状況になっちゃうからねぇ。
		//
		// ただし、C++ではgotoが変数宣言を飛び越える場合にかなり制限がかかるので、
		// C++でgotoを使う場合よーく注意しておくこと。
		...
		// まぁもしnoexcept指定してるのに関数で例外投げちゃったら、std::terminateなんてアプリ即死が起きます。
		// 後で述べるクラスのコピーコンストラクタやら代入演算子ってので必要になったりしますね。
...
#pragma region (嫌だけど)動的確保
	...
	void func8()
	{
		...
		// 理由はね、mallocを使うとクラスのコンストラクタが呼ばれないから

		// とりあえず試しにmallocでchar 10個分確保してみようか
		char* p = reinterpret_cast<char*>(std::malloc(sizeof(char)*10));
		// mallocはvoid*を返すのでC++だとキャストが必要なの。
		//
		// C言語では不要だけど、3つの理由でキャストされる場合があります。
		// 1. 大昔(C89以前)のmallocは、void*がなかったので cahr* malloc(int size)となっていて、char*から必要な型にキャストした時代があった
		// 2. C/C++の両方でビルドできるようにするため
		// 3. mallocに渡すサイズを間違えていた場合、キャストしているとコンパイラが警告を出してくれるから
		//    C言語でいうとこんな感じ char *p = (int*)malloc(sizeof(int));
		//
		// 1. はただの勉強不足とかやろ？・・・ていうか、web上でもC99まではキャストしないと使えないという嘘情報が堂々と記載されていたりね。。。
		//    ん？まさかC++でビルドしてるのをC言語と勘違いしているのか？そんな気もするページもあるぞ。はて？
		// 2. これは反対です。現在のC++はベターC言語といったものではないので、こんな理由でキャストするのはやめるべき。
		//    それに両方でキャストできるようにするにはC言語スタイルが必要だし、そもそもC++じゃmalloc使うなだし。
		// 3. これは有用な場合もあるだろうが、JPCERTのページに書かれているようなマクロ(次に記載するもの)を使うべきだと思う。だから直接手書きするのは反対かな。
		//    #define MALLOC(type) ((type *)malloc(sizeof(type))) // 単体で確保
		//    #define MALLOC_ARRAY(number, type) ((type *)malloc((number) * sizeof(type)))	// 配列で確保
		//
		// ただし、C言語編ではあえて書いてないけど、3. + C89(or C95) + stdlib.hのincludeを忘れた場合に問題が生じる可能性がある。
		// 上記条件ではmallocがintを返す関数と見なされるため、64bitの環境ではポインタのサイズがずれて問題になることがある。
		// ・・・多分、警告してくれると思うんだけど。
		// ※ C99からはプロトタイプ宣言必須なので stdlib.hがない場合はエラーとなると信じてる
		...
		*single = 10;
		...
		// あ、そうそうこっちは引数ありのコンストラクタが呼べないから、デフォルトコンストラクタかメンバ初期化できちんと初期化してないと不定値になるからね。
