...
#pragma region 列挙型
	...
	void func1()
	{
		...
		c = Color::Blue;	// 警告対策ｗついでに新形式のenumは名前空間を持つ(C++03では名前空間を持ってなくて苦労した)
...
#pragma region 構造体
	...
	void func2()
	{
		Struct c;	// 列挙型と同じくC++ではstruct Structとする必要はない。よってtypedefでの置き換えも不要
		...
		c = Struct();
		Struct d = { 0 };	// これだと、aが0でbが初期化されていればその値。初期化されてなければ0として初期化されます。
							// これは集成体型(配列とか構造体)全般に適用される話です。
							// C言語では構造体メンバの初期化値が与えれないので、{0}での初期化は良く使います。
							// memset使う人も大半が0初期化に使うので、この書き方を知らないのかな？と疑ってしまいます
		...
		{
			int i = 1;
			char str[7] = "sample";	// 普通は[]でコンパイラが計算するけど、構造体の場合ちょっと話がややこしいので、
									// 要素指定。Cのchar配列での文字列は文字数(1byte文字限定)+\0で7.
									// \0で文字列が終端するのがchar配列を使う場合の鉄則.
		};

		e f;
		f.i = 20;
		e g = f;	// <- これができることを知らない(これはそもそもC言語ですら出来る)のかmemcpyを使うって話です。
					//    酷いのになるとg.iにf.iを代入して、g.strをf.strにstrcpyしたりします。
					//    memcpyの場合、実数型で悲惨なことになります。0.0はall bit 0とは限らないからです。ま、使わなきゃいいだけ
	}
...
#pragma region if_繰り返し
	...
	void func3()
	{
		...
		// ※[]に比べて、.at()が要素のサイズオーバーを検出して例外をだすものになるのだけど、
		//   at()は、ひと昔前は例外を出したり検査するので少し遅いということで皆[]ばかりを使ってました。(at()の存在そのものを知らないなど思いたくない)
		//   今は.at()がconstexprというものになったので原則速度ペナルティはないです。
		//   よってこれからは.at()使いまくりで問題ないです。逆に極小の速度ペナルティ(constexprなのにあるのか？)を
		...
		for (std::size_t i = 0; i < vec.size(); i++) {
			std::cout << vec.at(i) << ' ';	// vecの内容を全部表示。この場合、絶対サイズオーバーしないので[]でも。。。いやいや、at()で
		}
		std::cout << std::endl;

		// range-based-for
		// もしarraysの要素を変更するならauto&&で。intだから値渡しで十分でしょ
		for (const auto element : arrays) {
			std::cout << element << ' ';		// arraysの内容を全部表示。arraysの要素は1つずつelementに格納される
		}
		std::cout << std::endl;
		...
		// これはVC6(1998年発売だった気がする。もう20年前かぁ)というものの弊害なのですけど。。。
		for (int i = 0; i < 10; i++) {
			// 変数iはこのループの中でのみ有効。ループの外に出るとiは無効となる。
		}
		// int j = i; <- これだと、iが定義されてないと怒られる。

		// ちなみにC言語でもC99では変数をどこでも宣言できるようになり、↑のforもfor内だけの
...
#pragma region typedef
	...
	void func4()
	{
		...
		// C言語交じりのソースではstructを例えばこのようにする必要があります。
		struct type b = { 0 };
		// C++じゃ必要ないんですが、このstructを付けるのが面倒だからか、
		// typedefでの付け替えがお約束です。
...
#pragma region namespace再び
	...
	void func6()
	{
		...
		// ただこれ、VS 2017だとプロパティでオプションの[c/C++]->[言語]->[C++言語標準] "ISO C++ 標準の最終草案(/std:c++latest)"
		// にしないと使えないよ
		// ↓みたいに名前空間をネストするんだなぁ
		int a = sample2_1::A::a;
...
#pragma region 例外
	...
	void func7()
	{
		...
		// finallyなしでどうやって後始末するの？とか思う人いるかもしれんが、C++はRTTIに基づいてオブジェクト
		...
		// というより、そもそも本当にメモリ不足になったとき、例外はもちろんログを残してなんてのだって出来ないことがほとんど。メモリが必要だから
...
#pragma region (嫌だけど)動的確保
	...
	void func8()
	{
		...
		// C言語では不要だけど、大昔のC言語で覚えて止まってしまった人がマレにいて、C言語なのにキャストする珍獣が
		// なぜかっていうと大昔のmallocは、void*がなかったので cahr* malloc(int size)ってなっていて、void*がないから
		// char*からキャストしてたっていうね(あぁ、そんな珍獣さっさと滅びればいいのに)
		...
		*single = 10;
		...
		// 使うことが出来ない。で、deleteの方に[]が付くのが要注意
		// あ、そうそうこっちはコンストラクタ呼べないから、デフォルトコンストラクタがきちんと初期化してないと不定値になるからね
		// ↓のでもmulti[3]ってやるまでmultiの領域は不定値になるから
