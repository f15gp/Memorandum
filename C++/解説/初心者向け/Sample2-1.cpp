...
#pragma region (嫌だけど)動的確保
	...
	void func8()
	{
		...
		// 1. 大昔(C89以前)のmallocは、void*がなかったので cahr* malloc(int size)ってなっていて、char*から必要な型にキャストした時代があった
		...
		// 3. これは有用な場合もあるだろうが、JPCERTのページに書かれているようなマクロ(次の記載するもの)を使うべきだと思う。だから直接手書きするのは反対かな。
