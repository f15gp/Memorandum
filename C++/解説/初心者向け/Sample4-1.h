...
#pragma region 4_1_1
	...
	class class_0
	{
		...
		// 2 コピーはディープコピーが基本(と、想定されている節がある。が、ポインタに限ってはシャローコピー)
		//   なので、生ポインタを使うと、くわすく知ってないと直ぐorz状態に陥る。
		//   が、生ポインタをクラス内部に持たないなら、大した話じゃない。
		// 3 関数は非virtual。
		//
		// 1については↓で示す
		// 2 はC言語から発展したことが関係するがC++のクラス、構造体の扱いを複雑にしてる理由(他言語と比べて。これはこれでいんじゃない？)。
		//	 ただ、JavaやC#みたいなシャローコピー(=ポインタのコピー)デフォも場合によっては問題だけど。
...
#pragma region 4_1_3
	...
	public:
		...
		//
		// 今は↓で書くように変数に初期値を与えれるのでそんなに必要ありません。
		...
		//
		// 大事なので言います。newで()付けなかったり、オブジェクトの宣言でhoge_4_1_3()じゃなくてhoge_4_1_3と書いてしまうと、
		// 明示的に↓の構文で初期化するかメンバイニシャライザで初期化されていない変数は全て不定値になります。ハマることあるので、理解するまでちゃんと初期化するように！
		...
		// 用途はhoge4_1_3(3L)なんてしたらboolの変数bはfalseとして扱われるってものですがね。
		// ただ、このデフォルト引数を使っちゃうと↓はlong xっていう一つの引数になっちゃいます。なので、まぁ詳しく理解するまでexplicitをこれにも付けなさいってね
		explicit hoge_4_1_3(long x, bool b = false) : a(static_cast<int>(x)) {}

		// あ、そうそう。引数付きコンストラクタを定義するとデフォルトコンストラクタは自動的に作られなくなるので、自分でかいてね
	...
	private:
		...
		//static int c;		// staticをメンバ変数に使うとこのクラス唯一の変数となります。いくらオブジェクトが存在しても一つだけ存在する変数になります。
...
#pragma region 4_1_4
	...
	public:
		...
		// さて何が頭痛いかというと、生ポインタとか持ってるとそれに対して適切な処理を書いてあげないといけない(ポインタはシャローコピー。でも普通値もコピーされることを期待するでしょ？)ことです。
		// 普通コピーなので新たにポインタに領域を用意してやってデータをコピーするような処理が必要です。
		...
		hoge_4_1_4(hoge_4_1_4&& rhs) {}
		hoge_4_1_4& operator=(hoge_4_1_4&& rhs) {}
		// やっぱり頭痛いことにポインタがあるとね・・・
...
#pragma region 4_1_5
	...
	public:
		...
		{
			v.push_back(n);
			u = std::make_unique<int[]>(n);
			u[n - 1] = 10;	// 確保した領域は0オリジンですから、-1しないとね
		}
		...
		// 昔はprivateで宣言してましたが、今は = defaultの代わりに=delete;と書けば未定義というかその実装は削除になります
	};
	...
	// 強い例外安全のためにありまして、コピーコンストラクタとかにはついておく必要がありますし、また例外を投げないように作成する
	// 必要があります。なぜ必要かと言うと、オブジェクトをコピーする位で例外を投げられると、コンパイラは困るからです。
