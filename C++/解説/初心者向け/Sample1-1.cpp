...
/// 概念はC#とかにもあるのでそんなに細かく書くこともないのだけど。。。
...
void sample1_1::func1()
{
	...
	// floatとdoubleは、IEEE754の32bitと64bitで大概実装されているためだが、この辺はJava、C#でも同じ。
	...
	// 計算の時はx87(=80bit)で計算するのですが、変数とかに代入するときdouble(=64bit)に丸めるため誤差が出る。
	// まぁ丸めないようにするアセンブラが使われていればいんだけど、そうじゃない時にハマるっていうレアパターンなんだよね。
	//
	// さらについでに、C言語やC++で数値を文字列に変換するとき、よくsprintfって関数を使います。
	...
	// それ以前のC言語の規格(C89とかC95)とC++では未定義動作です。・・・正確にはC++に関しては未定義動作でした。が正解です。
	// C++11が策定された時、参照C言語の規格がC99となったため、std::printfなどでは未定義動作ではなくなりました。
...
void sample1_1::func2()
{
	...
	// んでもC++11の前の規格であるC++03ではstatic_castのダブルとreinterpret_castが同じ結果になることは保証されていませんでした。
	// C++11からは保証されるので現代ではreinterpret_cast一択でOKです。
	double* ff = reinterpret_cast<double*>(&d);	// つまりこれでOK。(でも多用は禁物)
	// 
	// なお、継承関係にあるクラスを指すポインタでダウンキャスト(※1)を行うにはreinterpret_cast NGです。
	...
	*i = 200; // g, hが200になる
	...
	// 動的に行うんだが本当にそのクラスがダウンキャストできるか不安な場合にだけ使用する。
...
#pragma endregion 簡単なポインタ説明
	...
	*a = 19;
	...
	// 1. はスマートポインタ(まぁstd::unique_ptrとか)とvector または インスタンスとvector(or 配列)。インスタンスは後ほど説明
	...
	// いったんローカル変数にコピーしたりした方が高速化が確実に適用されます。だって、グローバル変数もどこかで誰かが変更したりするかも
	// しれないので、コンパイラさんは頑張れないのです

	// もし、ポインタに興味が出て、きちんと理解したいと思うなら、
	// "新・標準プログラマーズライブラリ C言語 ポインタ完全制覇 前橋 和弥" 2,678円とか買って読んでください。
	// 現存する本できちんとまとまってるのはこれ位じゃないんちゃいます？2,678円なんて勉強のためなら屁みたいなもんでしょ？
}
...
void sample1_1::func7(int a)
{
	...
	// 値渡しで十分だし早いのですが、考えるのが面倒の一言でそれらを全部参照渡しにしてもOK
